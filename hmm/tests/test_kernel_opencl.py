import hmm.kernel.opencl as opencl
...
ctx = opencl.Context(N, M)



class TestForwardNoScaling(unittest.TestCase):

    def test_belloch_is_correct_for_small_case(self):
        cl_alpha = opencl.forward_no_scaling_belloch(ctx, A, B, pi, ob)
        alpha    = numpy.zeros((T,N), numpy.float32)
        pyopencl.enqueue_copy(ctx.queue, alpha, cl_alpha)
        _, alpha2 = simple.forward_no_scaling(A, B, pi, ob)
        numpy.testing.assert_array_almost_equal(alpha, alpha2)

    def test_belloch_many_work_group_configurations(self):
    ...
    def test_belloch_1000_observation_len(self):
    ...
    def test_belloch_2000_observation_len(self):
        ob = numpy.loadtxt('data/t1.2000.dat', numpy.int16)
        _ = opencl.forward_no_scaling_belloch(ctx, A, B, pi, ob)

    def test_naive_is_correct_for_small_case(self):
        cl_alpha = opencl.forward_no_scaling_naive(ctx, A, B, pi, ob)
        alpha    = numpy.zeros((T,N), numpy.float32)
        pyopencl.enqueue_copy(ctx.queue, alpha, cl_alpha)
        _, alpha2 = simple.forward_no_scaling(A, B, pi, ob)
        numpy.testing.assert_array_almost_equal(alpha, alpha2)
    ...
    def test_naive_2000_observation_len(self):
        ob = numpy.loadtxt('data/t1.2000.dat', numpy.int16)
        _ = opencl.forward_no_scaling_naive(ctx, A, B, pi, ob)
