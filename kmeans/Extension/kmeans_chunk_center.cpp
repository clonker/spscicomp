...
int closest_center(PyArrayObject *point_data, PyArrayObject *centers, int cluster_size, int dimension)
{
	...
	{
	  distence += pow((*(double*)PyArray_GETPTR1(point_data, j)) - (*(double*)PyArray_GETPTR2(centers, i, j)), 2);	
	}
...
PyObject* kmeans_chunk_center(PyObject *data, PyArrayObject *centers)
{
    ...
    PyObject* returen_new_centers ;
    npy_intp dims[2] = {cluster_size,dimension};
    returen_new_centers = PyArray_SimpleNew(2, dims, NPY_DOUBLE);
    void *arr_data = PyArray_DATA((PyArrayObject*)returen_new_centers);
    memcpy(arr_data, new_centers, PyArray_ITEMSIZE((PyArrayObject*) returen_new_centers) * cluster_size * dimension);

    free(centers_counter);
    free(new_centers);
    return (PyObject*) returen_new_centers;
