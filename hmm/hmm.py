...
# Documentation convention PEP 257
...
def propagate(model, observation):
	...
	backwardCoeffs, _ = scaledBackwardCoeffs(model, observation, scalingFactors)
	...
	newInitialState[:] = alpha[0,:]*beta[0,:] / np.sum(alpha[0]*beta[0])
		...
		for j in range(0, len(newTransitionMatrix[i])):
			...
			for t in range(0, len(observation)-1):
				...
				denominator += transitionFrom(t, i, forwardCoeffs,
					backwardCoeffs)
#				denominator = alpha[t,:]*beta[t,:] / np.sum(alpha[t]*beta[t])
...
def scaledForwardCoeffs(model, observation):
	...
	for t in range(1,T):
		for i in range(0,N):
			alpha[t, i] = np.sum(alpha[t-1]*A[:,i])*B[i, observation[t]]
...
def scaledBackwardCoeffs(model, observation, scalingFactors):
	...
	for t in range(T-2,-1,-1):
		for i in range(0,N):
			...
			beta[t,i] = np.sum(A[i,:]*B[:,observation[t+1]]*beta[t+1])
		beta[t,:] *= c[t]
