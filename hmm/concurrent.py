...
class Task(object):
    def __init__(self, A, B, pi, ob, kernel, dtype):
        self.ob = ob
        self.A = A
        self.B = B
        self.pi = pi
...
def baum_welch_multiple(obs, A, B, pi, accuracy=1e-3, maxit=1000, kernel=spscicomp.hmm.kernel.python,
    ...
    pool = [Thread(tasks, results) for _ in xrange(num_threads)]
    for thread in pool:
        thread.start()
    ...
    while abs(new_eps - old_eps) > accuracy and it < maxit:
        for k in xrange(K):
            tasks.put(Task(A, B, pi, obs[k], kernel, dtype))
        for k in xrange(K):
            weights[k], nomsA[k], denomsA[k], nomsB[k], denomsB[k], gamma[k] \
                = results.get()

        A, B, pi = spscicomp.hmm.algorithms.update_multiple(
